--[[--
Export highlights to My Clippings and XMNote (纸间书摘).

Fork of the built-in exporter plugin, kept update-proof in a single plugin:
- only two targets: my_clippings and xmnote
- my_clippings gets Kindle-like stable in-book locations (#N), see clip.lua

Conventions:

- Target: each local format or remote service this plugin can translate to.
Each target inherits from "myexporter/base" and implements *at least* an export function.

- Highlight: Text or image in document. Stored in "annotations" table of documents sidecar file.
...
local MyClipping = require("myexporter/clip")
...
local targets = {
    my_clippings = require("myexporter/target/my_clippings"),
    xmnote = require("myexporter/target/xmnote"),
}
...
local MyExporter = WidgetContainer:extend{
    name = "myexporter",
...
function MyExporter:init()
    self.settings = G_reader_settings:readSetting("myexporter", {})
...
function MyExporter:onDispatcherRegisterActions()
    Dispatcher:registerAction("myexport_current_notes",
        {category="none", event="MyExportCurrentNotes", title=_("Export all notes in current book"), reader=true,})
    Dispatcher:registerAction("myexport_all_notes",
        {category="none", event="MyExportAllNotes", title=_("Export all notes in all books in history"), reader=true, filemanager=true})
end

function MyExporter:isReady()
...
function MyExporter:requiresNetwork()
    for k, v in pairs(self.targets) do
        if v:isEnabled() then
            if v.is_remote then
                return true
...
function MyExporter:onMyExportCurrentNotes()
...
function MyExporter:onMyExportAllNotes()
    if not self:isReady() then return end
    local clippings = {}
    clippings = updateHistoryClippings(clippings, self.parser:parseHistory())
    if Device:isKindle() then
        clippings = updateHistoryClippings(clippings, self.parser:parseMyClippings())
    end
...
function MyExporter:exportFilesNotes(files)
...
function MyExporter:exportClippings(clippings)
...
function MyExporter:addToMainMenu(menu_items)
    self.targets = genExportersTable(self.path)
    ...
    local menu = {
        text = "导出高亮（自制）",
        -- not listed in reader/filemanager_menu_order.lua (core files we
        -- don't touch): sort ourselves into the tools menu instead of
        -- floating around as an orphaned item
        sorting_hint = "tools",
        sub_item_table = {
            {
    ...
    menu_items.myexporter = menu
end

function MyExporter:setFilename()
...
function MyExporter:chooseFolder()
    ...
    local caller_callback = function(path)
        if current_path ~= path then
            self.ui.folder_shortcuts:updateShortcut(MyExporter.name, path)
            self.settings.clipping_dir = path
        end
    end
